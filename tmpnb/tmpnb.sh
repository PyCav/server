...
docker run --net=host -d -e CONFIGPROXY_AUTH_TOKEN=$TOKEN \
        ...
        --command="jupyter notebook --NotebookApp.base_url={base_path}  --NotebookApp.default_url='/tree' --ip=* --port {port}" # \
        #--NotebookApp.certfile='/etc/letsencrypt/live/evolve.ovh/fullchain.pem' --NotebookApp.keyfile='/etc/letsencrypt/live/evolve.ovh/privkey.pem'"
