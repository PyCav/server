...
while : ; do
	echo "Are you happy with the username "$user" (y/n)?"
	read answer
	if [ "$answer" == "y" ]; then
		break
	fi
...
while : ; do
	echo "Is "$site_name" the correct domain name (y/n)?"
	read answer
	if [ "$answer" == "y" ]; then
		break
	fi
...
echo "Would you like to install jupyterhub and nbgrader with raven authentication (y/n)?"
read jup
if [ "$jup" == "y" ]; then
	...
	while : ; do
		if [ "$port" == "" ] || [ "$port" == "443" ] || [ "$port" == "80" ]; then
			port=8000
		fi
		echo "Are you sure "$port" is the port you want to run JupyterHub on (y/n)?"
		read answer
		if [ "$answer" == "y" ]; then
			break
		fi
	...
	if [ "$answer" == "y" ]; then
		...
		read path
		while : ; do
			echo "Is "$path" the correct path to your Dockerfile (y/n)?"
			read answer
			if [ "$answer" == "y" ]; then
				break
			fi
	...
	read answer
	if [ "$answer" == "y" ]; then
		echo "Enter path (e.g. /media/backup/) for user data backups (empty string uses the home directory of the user you created earlier):"
		read backpath
		while : ; do
			if [ "$backpath" == "" ]; then
				backpath="/home/"$user"/"
			fi
			echo "Is "$backpath" the directory you want to store backups in (y/n)?"
			read answer
			if [ "$answer" == "y" ]; then
				break
			fi
